...
contract â€” its sole responsibility is to analyze a DEX bundle's
...
class itself contains almost no business logic â€” only orchestration. The
stage classes are:

    IngestStage, ParseStage, CounterfactualStage, ScoreStage,
    JudgmentStage, RebateStage, AppealStage.

Non-determinism budget (matrix-required â‰¥ 4 LLM, â‰¥ 3 fetches)
=============================================================
...
BPS_FAIR = 30          # â‰¤ 30 bps â†’ FAIR
BPS_BORDERLINE = 60    # 30 < bps â‰¤ 60 â†’ BORDERLINE
BPS_EXTRACTIVE = 150   # 60 < bps â‰¤ 150 â†’ EXTRACTIVE; > 150 â†’ PREDATORY
...
class MevFairCourt(gl.Contract):
    ...
    def _llm_clean_oracle_json(self, *, url: str, raw_doc) -> dict:
        """LLM site #1 â€” clean a fetched oracle blob into a canonical bundle
    ...
    def _llm_score_bundle(self, *, bundle_id: str, swaps: list, counterfactual: dict) -> dict:
        """LLM site #2 â€” score the bundle's extracted bps vs counterfactual."""
    ...
    def _llm_appeal_rescore(self, *, bundle_id: str, swaps: list, counterfactual: dict) -> dict:
        """LLM site #3 â€” rescore on appeal with a different oracle's CF."""
        swaps_text = json.dumps(swaps[:32], sort_keys=True)[:2048]
        cf_text = json.dumps(counterfactual, sort_keys=True)[:2048]
    ...
    def _llm_break_tie(self, *, bundle_id: str, old_bps: int, new_bps: int) -> dict:
        """LLM site #4 â€” tie-breaker between two close scores."""
        def call():
            prompt = (
    ...
    def _agree_on_error(self, leaders_res, call_fn) -> bool:
        ...
        try:
            ...
            # tuple-prefix equivalence: <STAGE:SEVERITY:...> â€” match the first two parts
    ...
    def _fetch_oracle_counterfactual(self, oracle_url: str):
        """Web fetch #1 â€” primary oracle counterfactual."""
        def call():
            try:
    ...
    def _fetch_appeal_oracle(self, url: str):
        """Web fetch #2 â€” appeal-time oracle (different URL)."""
        def call():
            try:
                response = gl.nondet.web.get(
    ...
    def _fetch_block_explorer(self, block_no: int, explorer_url: str) -> dict:
        """Web fetch #3 â€” sanity-check tx list for the bundle's block via a
        block explorer URL."""
    ...
    def withdraw_credit(self) -> int:
        ...
        # accounting only â€” actual value transfer happens off-chain per the
