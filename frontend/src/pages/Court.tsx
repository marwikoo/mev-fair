...
// labeled controls â€” input/select/textarea wrapped in their <label> for
...
// â”€â”€ stage timeline â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        {PIPELINE.map((st) => {
          ...
          return (
            <li key={st} className={`${s.tnode} ${on ? s.ton : ""}`}>
              ...
              <span className={s.tdetail}>{on ? detail.get(st) || "ok" : "â€”"}</span>
            </li>
          );
        })}
      </ol>
...
// â”€â”€ action card shell (collapsible accordion item) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ individual panels (module-level so input focus is stable) â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  return (
    ...
    >
      <div className={s.two}>
        ...
        <Field label="solver bond Â· GEN" value={bond} onChange={(e) => setBond(e.target.value)} inputMode="decimal" />
      </div>
      <Field label="bundle hash" value={hash} onChange={(e) => setHash(e.target.value)} />
      <Area label="swaps blob Â· json" value={swaps} onChange={(e) => setSwaps(e.target.value)} />
  ...
  return (
    ...
    >
      ...
      <div className={s.two}>
        ...
        <Field label="harm Â· bps" value={harm} onChange={(e) => setHarm(e.target.value)} inputMode="numeric" />
      </div>
      <Field label="complainant bond Â· GEN" value={bond} onChange={(e) => setBond(e.target.value)} inputMode="decimal" />
    </ActionCard>
  );
}
...
function PanelDisburse({ run, selectedId }: { run: RunFn; selectedId: string }) {
  return (
    <ActionCard
      ...
      desc="Credit victims pro-rata from the solver bond. (Re-runs the model â€” may need a retry.)"
      cta="Disburse rebate"
      disabled={!selectedId}
...
function PanelAppeal({ run, selectedId }: { run: RunFn; selectedId: string }) {
  ...
  return (
    ...
    >
      ...
      <Field label="appeal bond Â· GEN" value={bond} onChange={(e) => setBond(e.target.value)} inputMode="decimal" />
    </ActionCard>
  );
}
...
// â”€â”€ left rail â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function LeftRail({
  tracked,
  selectedId,
  onSelect,
...
// â”€â”€ center stage â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    <div className={s.stage}>
      ...
      <div className={s.stageSection}>
        <button className={s.collapse} onClick={() => setShowComplaints((v) => !v)} aria-expanded={showComplaints}>
          ...
          <span className="mono">{showComplaints ? "â€“" : "+"}</span>
        </button>
        {showComplaints &&
          (complaints.length === 0 ? (
...
// â”€â”€ bottom drawer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ page root â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
export function Court() {
  ...
  const run = useCallback<RunFn>(
    async (label, fn) => {
      ...
      if (wrongChain) {
        const m = "Wrong network â€” switch your wallet to studionet (chain 61999).";
        toast.push("error", m);
        return { ok: false, error: m };
      }
      const tid = toast.push("pending", `${label}â€¦`);
      try {
        await fn(client);
        toast.update(tid, "success", `${label} â€” done.`);
        refreshAll();
        return { ok: true };
      } catch (e) {
